...
public class Main {
	...
	public void onEntityJoinWorld(EntityJoinWorldEvent event) {
		if (event.entity != null && event.entity instanceof EntityPlayer && !event.entity.worldObj.isRemote) {
			...
			try {
				Default gm = getStatus(p.getName(), defaultGame);
				if (gm.getTotal_points() >= warningPoints) {
					Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentTranslation("§c[Warning] §7/ §a" + gm.getTotal_points() + " §7/ §b" + gm.getTitle()));
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
